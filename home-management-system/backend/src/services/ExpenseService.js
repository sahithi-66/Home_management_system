...
app.get('/api/expenses', async (req, res) => {
    ...
    try {
        ...
        const [expenses] = await connection.query(`
            SELECT e.id, e.description, e.amount, e.payer_id, u.username AS payer_name, e.category
            FROM expenses e
        ...
        const expensesWithSplits = await Promise.all(
            expenses.map(async (expense) => {
                ...
                const [splits] = await connection.query(`
                    SELECT us.username AS username, es.amount AS contributed_amount
...
app.post('/api/updateexpenses', async (req, res) => {
    const { expenseId, updatedData } = req.body;  // Destructure expenseId and updatedData from request body
    let connection;

    try {
        connection = await mysql.createConnection(dbConfig);
        // Update the expense record in the `expenses` table
        const updateExpenseQuery = `
            UPDATE expenses
            SET description = ?, amount = ?, payer_id = ?
            WHERE id = ?
        `;
        await connection.query(updateExpenseQuery, [
            updatedData.description,
            updatedData.amount,
            updatedData.payer_id,
            expenseId
        ]);
        
        // Update the splits for this expense in the `expense_splits` table
        // First, delete existing splits
        const deleteSplitsQuery = 'DELETE FROM expense_splits WHERE expense_id = ?';
        await connection.query(deleteSplitsQuery, [expenseId]);

        // Insert updated splits data
        const insertSplitsQuery = `
            INSERT INTO expense_splits (expense_id, user_id, amount)
            VALUES (?, ?, ?)
        `;
        console.log("Updated Splits:",updatedData.splits);
        const splitPromises = updatedData.splits.map(async (split) => {
            // const userId = await getUserId(split.username);
            ...
            if (!userId) {
                throw new Error(`User not found for username: ${split.username}`);
            }
            return connection.query(insertSplitsQuery, [
                expenseId,
                userId[0].id,
                split.contributed_amount,
            ]);
        });
        

        // Wait for all split inserts to complete
        await Promise.all(splitPromises);

        // Fetch the updated expense with splits
        const [updatedExpenses] = await connection.query(`
            SELECT e.id, e.description, e.amount, e.payer_id, u.username AS payer_name
            FROM expenses e
            JOIN users u ON e.payer_id = u.id
            WHERE e.id = ?
        `, [expenseId]);
        ...
        const [splits] = await connection.query(`
            SELECT us.username, es.amount AS contributed_amount
            FROM expense_splits es
            JOIN users us ON es.user_id = us.id
            WHERE es.expense_id = ?
        `, [expenseId]);

        const updatedExpenseWithSplits = { ...updatedExpenses[0], splits };

        // Send back the updated expense with splits
        res.json({ expenses: [updatedExpenseWithSplits] });
    } catch (error) {
...
app.post("/api/clearSplit/:id", async (req, res) => {
    ...
    try {
        ...
        const [debtorResult] = await connection.query(
            "SELECT net_amount FROM users WHERE username = ?",
            [split.debtor]
        );
        ...
        await connection.query(
            "UPDATE users SET net_amount = ?, is_debitor = ? WHERE username = ?",
        ...
        res.json({
            message: split.amount === 0 ? "Complete Split cleared successfully" : "Partial Split cleared successfully",
            updatedSplits: splits,
            clearedSplit,
        });
    } catch (error) {
